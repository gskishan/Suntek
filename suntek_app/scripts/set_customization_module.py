...
def execute():
    ...
    cf_count = frappe.db.sql("SELECT COUNT(*) FROM `tabCustom Field` WHERE module = 'suntek_app'")[0][0]
    ps_count = frappe.db.sql("SELECT COUNT(*) FROM `tabProperty Setter` WHERE module = 'suntek_app'")[0][0]

    print(f"Updated {cf_count} custom fields and {ps_count} property setters to module 'suntek_app'")

    # Fixed these lines - removing the split() calls
    cf_excluded = frappe.db.sql(
        f"SELECT COUNT(*) FROM `tabCustom Field` WHERE name IN ('{excluded_cf_str}') AND module IS NULL"
    )[0][0]
    ps_excluded = frappe.db.sql(
        f"SELECT COUNT(*) FROM `tabProperty Setter` WHERE name IN ('{excluded_ps_str}') AND module IS NULL"
    )[0][0]

    print(
        f"Confirmed {cf_excluded}/{len(excluded_custom_fields)} custom fields and {ps_excluded}/{len(excluded_property_setters)} property setters excluded"
    )
