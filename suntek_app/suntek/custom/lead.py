...
def verify_channel_partner(user):
    if frappe.db.exists("Channel Partner", {"linked_user": user}):
        cp = frappe.get_doc("Channel Partner", {"linked_user": user})

        return {"channel_partner_name": cp.name}
...
def save_name_changes_to_contact(doc, method=None):
    contact = frappe.db.get_value("Contact", {"mobile_no": doc.mobile_no}, ["name"], as_dict=1)
    if not contact:
        return
