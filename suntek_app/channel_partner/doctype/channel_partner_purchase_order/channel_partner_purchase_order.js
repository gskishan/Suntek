...
function populate_from_sales_order(frm, data) {
	...
	frappe.show_alert({
		message: __("Items fetched from Sales Order {0}. Please add taxes manually.", [
			data.sales_order,
		]),
		indicator: "green",
	});
...
function populate_from_quotation(frm, data) {
	...
	if (data.taxes_and_charges_template) {
		...
		// This will trigger the taxes_and_charges_template event to populate taxes
	}
	...
	frappe.show_alert({
		message: __("Items fetched from Quotation {0}. Please add taxes manually if needed.", [
			data.quotation,
		]),
		indicator: "green",
	});
