frappe.ui.form.on("Lead", {
  ...
  before_save: function (frm) {
    if (frm.doc.mobile_no) {
      if (frm.doc.mobile_no.includes(" ")) {
        frm.doc.mobile_no = frm.doc.mobile_no.replace(/\s+/g, "");
      }
    }
  },
  source: function (frm) {
  ...
  setup: function (frm) {
    if (frm.is_new()) {
      if (
        frm.doc.source === "Digital Marketing" &&
        !frm.doc.custom_department
      ) {
        frm.set_value("custom_department", "Tele Sales - SESP");
      }
    }
            ...
            callback: function (r) {
              if (!r.exc) {
                frappe.model.open_mapped_doc({
                  method:
                    "suntek_app.suntek.custom.lead.custom_make_opportunity",
                  frm: frm,
                });
              }
              d.hide();
